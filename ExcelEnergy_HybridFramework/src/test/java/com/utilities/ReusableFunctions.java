...
//	}
	...
	public void sendKeysByUsingExcelData(By locator, int rowNum, int cellNum) throws Exception {
		fi = new FileInputStream(".\\src\\test\\resources\\testdata\\Excel_testdata.xlsx");
		wb = new XSSFWorkbook(fi);
		// Read the sheet from workbbok
		...
		if (driver.findElements(locator).size() > 0) {
			...
			if (element.isEnabled()) {
				...
				// Send the test data to Edit box
				highlightElement(element);
				element.sendKeys(c.getStringCellValue());
			} else {
				System.out.println("Given locator is not enable state on DOM(Current page***");
			}
		} else {
...
//		}
	...
	public String getRandomEmailAsString() {
